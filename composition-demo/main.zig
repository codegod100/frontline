...
fn renderGreeting(comp: *Component, allocator: std.mem.Allocator) !*VNode {
    ...
    try button.setProp("style", "margin-top: 16px; padding: 8px 16px; background: #10b981; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; transition: background 0.2s;");
    try button.setProp("onclick", "window.wasmModule.exports.updateGreeting()");
...
fn renderCounter(comp: *Component, allocator: std.mem.Allocator) !*VNode {
    ...
    try button.setProp("onclick", "window.wasmModule.exports.incrementCounter()");
    ...
    try button2.setProp("onclick", "window.wasmModule.exports.decrementCounter()");
